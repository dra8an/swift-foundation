...
public struct RootCollator: @unchecked Sendable {
    ...
    /// Compare dispatch resolved once at init, so the per-call path has no
    /// #available branch (which is an optimization barrier on the classic
    /// fast-Latin path). Non-capturing + `borrowing` self => no per-call ARC.
    private let compareImpl: @Sendable (borrowing RootCollator, String, String, CollationOptions) throws -> Order

    private static func resolveCompareImpl()
        -> @Sendable (borrowing RootCollator, String, String, CollationOptions) throws -> Order {
        if #available(macOS 26.0, iOS 26.0, watchOS 26.0, tvOS 26.0, visionOS 26.0, *) {
            return { c, l, r, o in try c.compareWithSpan(l, r, options: o) }
        }
        return { c, l, r, o in try c.compareClassic(l, r, options: o) }
    }
    ...
    public init(data: CollationData, norm: NormalizationData) {
        ...
        self.compareImpl = RootCollator.resolveCompareImpl()
    }
    ...
    public func compare(
        ...
        try compareImpl(self, left, right, options)
    }
