...
struct NumericTests {
    ...
    private func cmp(_ a: String, _ b: String) -> Int {
    ...
    @Test func denseEncodingBoundary() throws {
        ...
        for i in 1..<ordered.count {
            #expect(cmp(ordered[i - 1], ordered[i]) < 0, "\(ordered[i-1]) < \(ordered[i])")
            #expect(cmp(ordered[i], ordered[i - 1]) > 0)
        }
    ...
    @Test func sevenToEightDigitBoundary() throws {
        ...
        #expect(cmp("9999999", "10000000") < 0)
        #expect(cmp("10000000", "9999999") > 0)
        try keysOrderedSame(["9999998", "9999999", "10000000", "10000001"])
    }

    @Test func leadingZerosAreEqual() {
        #expect(cmp("00012", "12") == 0)
        #expect(cmp("0", "000") == 0)
        #expect(cmp("a001042490z", "a1042490z") == 0)
    }

    @Test func longDigitRuns() throws {
        // Well past 7 significant digits: pair encoding with multiple CEs.
        #expect(cmp("12345678901234567890", "12345678901234567891") < 0)
        #expect(cmp("099999999999999999999", "100000000000000000000") < 0)
        try keysOrderedSame(["99999999999999999999", "100000000000000000000"])
    }

    @Test func trailingZeroPairs() {
        // The pair encoding trims trailing 00 pairs; equal numbers with
        // different textual lengths via leading zeros must stay equal.
        #expect(cmp("1200000000", "01200000000") == 0)
        #expect(cmp("1200000000", "1200000001") < 0)
    }

    @Test func digitRunsInContext() {
        // Runs embedded in text (the Finder file-name case).
        #expect(cmp("file9.txt", "file10.txt") < 0)
        #expect(cmp("IMG_20240115.jpg", "IMG_20240116.jpg") < 0)
        #expect(cmp("v1.9.9", "v1.10.0") < 0)
    }

    @Test func numericSearchStillMatches() {
