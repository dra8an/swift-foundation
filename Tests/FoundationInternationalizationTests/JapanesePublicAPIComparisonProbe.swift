...
private struct JapanesePublicAPIComparisonProbe {
    ...
    private static func makePair(timeZone: TimeZone = .gmt,
        ...
        let icuInner = _CalendarICU(
            identifier: .japanese, timeZone: timeZone, locale: nil,
    ...
    private static func compareSurfaces(label: String, date d: Date,
        ...
        let constitutionDC = DateComponents(month: 5, day: 3)
    ...
    // MARK: - Topic 1: public-API sweep across modern eras

    @Test func publicAPI_sweepAllMethods() {
        let (icu, ours) = Self.makePair()
        let div = Divergences()
        // Anchors stay safely inside a single era so era/year round-trips remain stable.
        let dates: [(String, Date)] = [
            ("Meiji 1900-06-15 noon",   Self.g(1900, 6, 15)),
            ("Taisho 1920-06-15 noon",  Self.g(1920, 6, 15)),
            ("Showa 1968-03-15 noon",   Self.g(1968, 3, 15)),
            ("Heisei 1999-06-15 noon",  Self.g(1999, 6, 15)),
            ("Heisei 2016-09-23 noon",  Self.g(2016, 9, 23)),
            ("Reiwa 2020-02-29 noon",   Self.g(2020, 2, 29)),
            ("Reiwa 2024-10-15 noon",   Self.g(2024, 10, 15)),
            ("Reiwa 2025-07-04 noon",   Self.g(2025, 7, 4)),
            ("Reiwa 2026-06-11 noon",   Self.g(2026, 6, 11)),
            ("Reiwa 2050-01-15 noon",   Self.g(2050, 1, 15)),
        ]
    ...
    // MARK: - Topic 4: Gregorian year boundaries within a single era

    @Test func yearBoundaries_commonAndLeap() {
        let (icu, ours) = Self.makePair()
        let div = Divergences()
        var dates: [(String, Date)] = []
        // All inside a single era (Heisei or Reiwa) to avoid era-wrap noise.
        for y in [2000, 2020, 2024, 2096] {
            dates.append(("\(y)-01-15 00:00", Self.g(y, 1, 15, hour: 0)))
            dates.append(("\(y)-12-15 23:59", Self.g(y, 12, 15, hour: 23, minute: 59, second: 59)))
    ...
    // MARK: - Topic 7: era boundary public-API behaviour (Japanese-specific)

    @Test func eraBoundaries_publicAPI() {
        let (icu, ours) = Self.makePair()
        let div = Divergences()
        // Each pair brackets a modern era transition with public-API surface checks.
        let dates: [(String, Date)] = [
            ("Meiji last (1912-07-29)",   Self.g(1912, 7, 29)),
            ("Taisho 1 (1912-07-30)",     Self.g(1912, 7, 30)),
            ("Taisho last (1926-12-24)",  Self.g(1926, 12, 24)),
            ("Showa 1 (1926-12-25)",      Self.g(1926, 12, 25)),
            ("Showa last (1989-01-07)",   Self.g(1989, 1, 7)),
            ("Heisei 1 (1989-01-08)",     Self.g(1989, 1, 8)),
            ("Heisei last (2019-04-30)",  Self.g(2019, 4, 30)),
            ("Reiwa 1 (2019-05-01)",      Self.g(2019, 5, 1)),
        ]
        for (label, date) in dates {
            Self.compareSurfaces(label: label, date: date, icu: icu, ours: ours, div: div)
        }
        Self.reportAndAssert("eraBoundaries_publicAPI", div, dateCount: dates.count)
    }
