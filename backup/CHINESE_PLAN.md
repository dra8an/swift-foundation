...
| W1 | FUTURE WATCHPOINT, not a current divergence: upstream ICU 78.1 (Oct 2025, ICU-23167, confirmed Done via Jira 2026-07-23) changed UCAL_EXTENDED_YEAR for chinese/dangi/ethiopic-amete-alem to Temporal's arithmetical year; UCAL_ERA and UCAL_YEAR unchanged | yearForWeekOfYear FIELD values (= ext) once Apple rebases onto ICU 78.1+ | Apple's shipped ICU predates the change; our port matches Apple by design | When Apple rebases: the parity target shifts under us; era/year/cycle fields unaffected; expected fix is the single extOffset constant (2637) plus probe expectations. Watch Apple ICU rebases |
...
|---|---|---|
| R1 | ±66M month-add bound miscalibrated vs ±5M ext domain — 66M months ≥ 5.08M yrs even at 13-mo/yr floor, so near-limit adds necessarily produce exts that date(from:)/year-add reject; plus the O(n) walk = hours of astronomy (Calendar_Chinese.swift:1028) (CONFIRMED) | ✅ FIXED (`84f6516`): named constants + ext-aware /13 reach precheck + exact post-walk domain guard. O(n) walk stays S4 |
| R2 | yearForWeekOfYear add = second O(n)-astronomy hang instance — |n| ≤ ~5M valid iterations × ~50–200 µs/fallback-year = minutes-to-tens-of-minutes on one call (:1062) (CONFIRMED) | ✅ FIXED (`44a08a0`): telescoped to O(1) anchor difference; dead numWeeksInYearForWeekOfYear removed |
| R3 | 3 new quarter comments manually wrapped mid-sentence, violating guideline "Do not manually wrap comments or DocC." (:699-703, :509-511, :544-545); same-range week-year comments correctly unwrapped (CONFIRMED) | ✅ FIXED (`ccd76f4`): all three joined to single logical lines |
| R4 | Quarter model duplicated: dateInterval walks ×3, range(.month,.quarter) re-derives + walks ×2; formulas (m+2)/3, 3(q−1)+1, (m−1)%3+1 across 4 sites (:508/:698) (CONFIRMED) | ✅ FIXED: quarterSpan(ext:ordinal:) helper feeds both sites; formulas live in one place |
| R5 | Quarter parity gate gaps: GMT-only (floor-seconds (.day,.quarter) never DST-checked; the one DST probe omits .quarter deliberately) + no nextDate/enumerate quarter coverage (dateAfterMatchingQuarter untested for chinese) (CONFIRMED; 3rd claimed gap REFUTED — stride hits displaced post-leap months in 1900/1957/2044/2071) | ✅ FIXED: quarterSurfacesDSTAndMatching test (LA-tz surfaces + nextDate q1-4 GMT/LA). ⚠ CAUGHT A REAL DIVERGENCE: ICU range(.day,.quarter) counts calendar days, our generic fallback overcounted by 1 in DST fall-back quarters → dedicated RD-based case added (also closes R10) |
...
| R7 | .yearForWeekOfYear interval hand-rolls RD→Date+DST conversion ×2 instead of in-file utcDate(fromRataDie:) — verified bit-identical drop-in; only interval case bypassing the helper (shape copied from Hebrew, which shares the flaw) (:675) (CONFIRMED) | ✅ FIXED: two utcDate(fromRataDie:) calls; Hebrew mirror = separate upstream cleanup, noted |
| R8 | ChinesePackingExperiment runs multi-second benches every suite run for a settled decision (VarA verified byte-identical to shipping table) (CONFIRMED) | ✅ FIXED: @Test(.enabled(if: CHINESE_PACKING_EXPERIMENT env set)) — skipped by default, file kept per § 11.19 |
...
| R10 | range(.day,.quarter) via generic fallback rebuilds the quarter interval 3× (:524); cold path, low severity; ⚠ naive dedicated-case fix is off-by-one in DST fall-back quarters — must keep floor-seconds-at-end−0.1 semantics (CONFIRMED) | ✅ FIXED as a side effect of R5: dedicated RD-based (.day,.quarter) case — zero interval computations AND the ICU-correct day count (the verifier's floor-seconds advice was wrong: the DST probe shows ICU counts calendar days) |
