...
10b. **`backup/LOCAL_VS_UPSTREAM_DIVERGENCE.md`** ⚠ NEW 2026-06-10 — tracks the two places local `port/hebrew` diverges from upstream's PR-shipped code (PR #2028). Both divergences are intentional probes that restore per-pattern fall-through after the reviewer's opt-in flag design removed it. **Must be preserved across every future back-sync.** Without these probes, 14 Suite B tests fail. Includes the upstream-vs-local code for each divergence, sync discipline guidance, and the path to eventually feed back upstream.
