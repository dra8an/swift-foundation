...
// Baseline (ICU-backed `.hebrew`), debug mode, 2026-04-23, Intel iMac 2019, macOS 15.7.3, Swift 6.3.1.
// Methodology: best of 5 runs (isolates min from system noise).
//   allocationsForFixedHebrewCalendar: min 21,321 ns/iter        (max 42,171)
//   roundTripDateComponents:           min 20,346 ns/round-trip  (max 53,569)
//   copyOnWriteHebrew:                 min 69,590 ns/iter        (max 99,333)
//   nextThousandHanukkahs:             min 776 µs/match          (max 2,291)
// The min/max spread shows run-to-run variance (up to 3× wide on a busy machine).
// The `_CalendarHebrew` port must improve the `min` values to demonstrate a real speedup.
// Release-mode numbers unavailable on this machine: `swiftpm-testing-helper` crashes with SIGBUS
// on Intel x86_64 + Swift 6.3.1 before our tests can produce output. Debug-mode ratios (before-port
// vs after-port) remain meaningful because both runs pay the same debug-overhead tax.
