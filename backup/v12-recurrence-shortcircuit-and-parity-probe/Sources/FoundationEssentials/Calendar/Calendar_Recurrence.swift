...
extension Calendar {
    ...
    fileprivate func _unadjustedDates(after startDate: Date,
        ...
        // Single-combination fast-path: when every populated combination field
        // has exactly one value AND the policies are at default, the entire
        // expansion-and-filter chain below collapses to a single
        // `_calendarNextDate` call. We probe the calendar implementation; if
        // it has a fast-path answer, we skip the chain entirely. This benefits
        // simple yearly/monthly RecurrenceRule patterns like
        // `RecurrenceRuleThanksgivings` and `RecurrenceRuleLaborDay`.
        // Other calendars get the protocol default (`nil`) and fall through.
