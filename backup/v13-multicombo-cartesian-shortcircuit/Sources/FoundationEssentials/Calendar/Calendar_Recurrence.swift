...
extension Calendar {
    ...
    ///   would exceed `maxCombinations` (defensive cap against pathological
    ///   inputs);
    /// - any weekday entry is `.every(day)` (no ordinal) or `.nth(N, day)`
    ///   with `N <= 0` — these don't translate cleanly to a single
    ///   fast-path DateComponents (`.every` requires `{weekday}`-only
    ///   semantics with same-day-time-of-day handling that our fast path
    ///   doesn't support; negative ordinals diverge from ICU's raw
    ///   enumerate behavior).
    ///
    /// On success, the returned array contains every cartesian combination
    /// of populated fields. Each produced `DateComponents` is fast-path-
    /// eligible (positive ordinal, single value per field).
    fileprivate func _expandedDateComponents(
        _ c: _DateComponentCombinations,
        maxCombinations: Int = 64
    ) -> [DateComponents]? {
        // Reject patterns we can't fast-path cleanly.
        ...
        // Single-combination case is handled by _singleCombinationDateComponents;
        // only use this multi-combo path when there is more than one DC to
        // produce (otherwise the single-combo path is more direct).
        ...
        func make(monthIdx: Int, weekdayIdx: Int,
            ...
            if let wds = c.weekdays {
                if case .nth(let n, let day) = wds[weekdayIdx] {
                    dc.weekdayOrdinal = n
                    dc.weekday = day.icuIndex
                }
                // .every is rejected above; this switch is exhaustive otherwise.
            }
    ...
    /// (if any) is `.nth(N, day)` with `N >= 1`, return the equivalent
    /// `DateComponents`. Otherwise return `nil` to indicate the combinations
    /// require expansion. Used by `_unadjustedDates`'s fast-path probe.
    fileprivate func _singleCombinationDateComponents(
        ...
        if let weekday = c.weekdays?.first {
            switch weekday {
            case .every:
                // Plain "every <day>" — no ordinal. Could be fast-pathed via
                // {weekday}, but only when no other day-positioning fields are
                // present. Punt to the existing path for now.
                return nil
    ...
    fileprivate func _unadjustedDates(after startDate: Date,
        ...
        // (2) Multi-combination short-circuit: cartesian product of fields
        // produces N >= 2 fast-path-eligible `DateComponents`. Each is
        // probed via `_calendarNextDate`; if ALL succeed, we sort the
        // resulting dates chronologically and return — skipping the
        // expansion-and-filter chain. Benefits e.g.
        // `RecurrenceRuleThanksgivingMeals` (multi-hour). Falls through
        // for `RecurrenceRuleBikeParties` (negative ordinals — rejected by
        // `_expandedDateComponents`) and `RecurrenceRuleDailyWithTimes`
        // (`.every(day)` weekdays — also rejected).
        if matchingPolicy == .nextTime && repeatedTimePolicy == .first,
