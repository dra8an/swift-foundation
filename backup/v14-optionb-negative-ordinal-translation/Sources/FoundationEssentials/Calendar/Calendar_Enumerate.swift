...
extension Calendar {
    ...
    struct DatesByMatching : Sendable, Sequence {
        ...
        struct Iterator: Sendable, IteratorProtocol {
            ...
            // Fast-path commit: when the calendar implementation can answer
            // `nextDate(after:matching:direction:)` directly AND policies are
            // at default, we skip `_enumerateDatesStep` (and its per-call
            // policy recheck + SearchStepResult build) and call the protocol
            // method straight from `next()`. Probed once at init.
            ...
            mutating func next() -> Element? {
                ...
                if usesFastPath {
                    // Skip the framework: ask the calendar directly. The probe
                    // in init confirmed the pattern is recognized; returning
                    // nil here means "no more matches in this direction" — the
                    // sequence terminates.
...
extension Calendar {
    ...
    internal func dateAfterMatchingMonth(startingAt startDate: Date, components: DateComponents, direction: SearchDirection, strictMatching: Bool) throws -> Date? {
        ...
        if month != dateMonth {
            // Fast-path: ask the calendar implementation directly. Pass minimal
            // components so our fast path matches the helper's "advance to start
            // of target month, ignore other fields" semantic. The default
            // protocol impl returns nil; other calendars fall through.
            // We handle leap-month-desired strict-matching cases via the
            // existing logic below, so skip the fast-path when that's set.
    ...
    internal func dateAfterMatchingWeekOfMonth(startingAt: Date, components: DateComponents, direction: SearchDirection) throws -> Date? {
        ...
        // Fast-path: enrich components with the date's current month and ask
        // the calendar directly. This routes through the {month, weekday,
        // weekOfMonth} fast path which has been verified to match ICU exactly.
        // We only attempt this when weekday is set (otherwise the helper's
        // "preserve weekday from start" semantic isn't representable as a
        // single-DateComponents query).
        // If the fast-path can't satisfy the request (e.g. the constrained
        // month doesn't contain the target weekday in the target week), it
        // returns nil and we fall through to the existing slow walk.
    ...
    internal func dateAfterMatchingWeekdayOrdinal(startingAt: Date, components: DateComponents, direction: SearchDirection) throws -> Date? {
        ...
        // Fast-path: ask the calendar implementation directly. We've already
        // confirmed startingAt does not match (would've returned nil above), so
        // strictly-after semantics is what we want. The default protocol impl
        // returns nil, so other calendars fall through to the existing walk.
    ...
    internal func dateAfterMatchingWeekday(startingAt: Date, components: DateComponents, direction: SearchDirection) throws -> Date? {
        ...
        // Fast-path: ask the calendar with minimal {weekday}-only components.
        // The helper only inspects components.weekday, so passing the full
        // input would over-constrain our fast path. The default protocol impl
        // returns nil; other calendars fall through to the existing walk.
        var minimal = DateComponents()
