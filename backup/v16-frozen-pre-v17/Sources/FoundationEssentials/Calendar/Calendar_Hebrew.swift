...
internal final class _CalendarHebrew: _CalendarProtocol, @unchecked Sendable {
    ...
    package func nextDate(after date: Date, matching components: DateComponents,
        ...
        // {month, weekday, weekOfMonth} is the only allowed weekOfMonth pattern.
        // The no-month form is rejected because ICU's enumerate diverges in the rare
        // cases where a particular weekday isn't reachable in the requested week
        // (e.g. Mon in week 1 of a month starting on Tue with minDaysInFirstWeek=1).
        if hasWeekOfMonth && !(hasMonth && hasWeekday && !hasDay && !hasWdOrd) { return nil }
        // weekdayOrdinal is allowed when paired with weekday — month optional. {wd,wdOrd}
        // (no month) finds the Nth weekday-occurrence in the current civil month, advancing
        // months as needed. Rejected when day is set or weekOfMonth is set.
    ...
    /// Fast path for `{h, mi, s, ns?}` — find the next/prev date strictly after
    /// `date` whose time-of-day equals the requested time. Same-day match if the
    /// target is strictly after `currentSecsInDay`; otherwise advance one RD.
    /// `_adjustedDate` runs after this in `_dates` to apply DST corrections, so
    /// we use `.former` policies here matching the other fast paths.
    ...
    /// Fast path for `{weekday, weekdayOrdinal, h?, m?, s?, ns?}` (no `month`) —
    /// finds the Nth occurrence of the weekday in the date's current civil
    /// month, advancing month-by-month if the ordinal is out of range. Used by
    /// `_unadjustedDates` which has already positioned `date` in the search
    /// month via `dateAfterMatchingMonth`. O(1) per candidate month.
    private func nextWeekdayOrdinalMatch(
        ...
        // Compute the day-of-month satisfying (weekday, ordinal) for `(year, civilMonth)`,
        // or nil if month is invalid (Adar I in common year) or the ordinal is out of range.
        ...
        // Walk months (forward or backward) until we find the target ordinal in range
        // AND the candidate is strictly after / before the input.
        var y = currentYear
        var cm = currentCivilMonth
        // Bound generously: at most 14 steps covers a Hebrew leap cycle's worth of
        // months, including the leap-only-Adar-I skip.
        for _ in 0..<14 {
            ...
            // Advance / retreat civil month, skipping the Adar-I-only-in-leap-years slot.
            if forward {
    ...
    /// Fast path for `{month, weekday, weekOfMonth, h?, m?, s?, ns?}` —
    /// "Thursday in the 4th week of November" / "Tuesday in week 2 of Adar".
    /// Inverts the calendar-agnostic ICU week-numbering algorithm (matching
    /// `weekNumber(...)` in this file) to find the day-of-month directly,
    /// in O(1) per candidate year.
    private func nextMonthWeekdayWeekOfMonthMatch(
        ...
        guard targetMonth >= 1, targetMonth <= 13 else { return nil }
        guard targetWeekday >= 1, targetWeekday <= 7 else { return nil }
        // Negative or zero weekOfMonth not part of ICU's enumerate matching contract.
        ...
        // Compute the day-of-month that satisfies (weekOfMonth, weekday) for `(year, civilMonth)`,
        // or nil if the month is invalid this year (Adar I in common years) or the
        // computed day falls outside the month.
...
internal enum HebrewArithmetic {
    ...
    ///
    /// A 4-slot version was tried (2026-05-04) and reverted: gave 2–5% wins on
    /// year-crossing benchmarks but 5–6% regressions on `nextThousand*`
    /// fast-path benchmarks where the extra slots-to-scan was pure overhead.
