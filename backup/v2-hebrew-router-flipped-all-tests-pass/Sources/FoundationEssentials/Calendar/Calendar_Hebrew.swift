...
internal final class _CalendarHebrew: _CalendarProtocol, @unchecked Sendable {
    ...
    func date(byAdding components: DateComponents, to date: Date, wrappingComponents: Bool) -> Date? {
        ...
        if daysToAdd != 0 {
            // DST-aware: walk civil days in local time (never assume 86400 seconds per day).
            let tz = self.timeZone
            let localComps = dateComponents([.year, .month, .day, .hour, .minute, .second, .nanosecond], from: result, in: tz)
            guard let y = localComps.year, let m = localComps.month, let d = localComps.day else { return nil }
            var (ny, nm, nd) = (Int32(y), UInt8(m), UInt8(d))
            var remaining = daysToAdd
            while remaining > 0 {
                (ny, nm, nd) = Self.nextDay(year: ny, civilMonth: nm, day: nd)
                remaining -= 1
            }
            while remaining < 0 {
                // Walk back: previous day is (day-1) if day > 1, else last day of previous month.
                if nd > 1 {
                    nd -= 1
                } else {
                    var pm = nm - 1
                    ...
                    nm = pm
                    ny = py
                    nd = HebrewArithmetic.daysInCivilMonth(year: ny, civilMonth: nm)
                }
                remaining += 1
            }
            ...
            newDC.timeZone = tz
            guard let newDate = self.date(from: newDC) else { return nil }
            result = newDate
        }
