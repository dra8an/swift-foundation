...
//
// The "allocations" and "CoW" speedups are modest because those paths are dominated
// by Foundation's Calendar-struct plumbing (copy-on-write, locale lookup) rather than
// the Hebrew-specific math we replaced. `date(byAdding: .day)` does proper DST-aware
// component math (unpack → increment civil day → repack) which is ~20 ns in ICU
// vs ~1 µs for us in pure-Swift — but it's correct across DST transitions.
//
