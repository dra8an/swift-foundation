...
/// by `recurrences(of:)` between Foundation's ICU-backed Hebrew calendar
/// (`Calendar(identifier: .hebrew)` when the pure-Swift feature flag is OFF —
/// the default outside `FOUNDATION_FRAMEWORK`) and `_CalendarHebrew`.
///
...
private struct HebrewRecurrenceRuleParityProbe {
    ...
    private static func makePair(timeZone: TimeZone = .gmt) -> (icu: Calendar, ours: Calendar) {
        // ICU side: `Calendar(identifier: .hebrew)` routes to `_CalendarICU`
        // whenever the pure-Swift Hebrew feature flag is OFF — the default
        // outside `FOUNDATION_FRAMEWORK` (e.g. this SPM build). No direct
        // `_CalendarICU(...)` reference is needed in this file, which keeps
        // it upstream-safe.
