...
extension Calendar {
    ...
    /// Cheap structural check: does the combinations contain any
    /// `.nth(N, day)` weekday entry with `N < 0`? Used by
    /// `_unadjustedDates` to gate the negative-ordinal-translation path.
    ...
    fileprivate func _expandedDateComponents(
        ...
        // For negative-ordinal translation, precompute the target month's
        // structure once (one set of calendar primitive calls per
        // `_unadjustedDates` invocation, only when the pattern actually
        // contains a negative ordinal).
        struct MonthInfo {
            let month: Int
        ...
        if hasNegativeOrdinal, let anchor = anchor {
            // Determine the target (year, month) for negative-ordinal translation.
            let targetMonth: Int
            ...
            if let ms = c.months, ms.count == 1 {
                ...
                // Monthly recurrence: target = anchor's month.
                targetMonth = self.component(.month, from: anchor)
                targetIsLeap = false
                targetYear = self.component(.year, from: anchor)
            } else {
                // Multi-month + negative ordinal: not yet supported.
                return nil
            }
        ...
        // Translate a `.nth(N, day)` entry to its DC fields. Returns nil if
        // the negative-ordinal target falls outside the month (e.g. asking
        ...
        func translateWeekday(_ entry: RecurrenceRule.Weekday, into dc: inout DateComponents) -> Bool {
            ...
            // Negative ordinal: translate to {month, weekday, weekOfMonth}.
            ...
            // Compute weekOfMonth using the ICU calendar-agnostic formula.
