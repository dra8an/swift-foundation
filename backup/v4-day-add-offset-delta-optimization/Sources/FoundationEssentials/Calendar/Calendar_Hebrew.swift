...
internal final class _CalendarHebrew: _CalendarProtocol, @unchecked Sendable {
    ...
    func minimumRange(of component: Calendar.Component) -> Range<Int>? {
        ...
        case .year: return 1..<140743              // matches Gregorian's range
        ...
        case .weekdayOrdinal: return 1..<5
        case .quarter: return 1..<5
        case .weekOfMonth: return 1..<5
        case .weekOfYear: return 1..<52            // ≈53 weeks in a common year
        case .yearForWeekOfYear: return 140742..<140743
        case .nanosecond: return 0..<1_000_000_000
        case .isLeapMonth: return 0..<1            // Hebrew always reports false
        case .isRepeatedDay: return 0..<1
        case .dayOfYear: return 1..<354            // shortest Hebrew year is 353 days
    ...
    func maximumRange(of component: Calendar.Component) -> Range<Int>? {
        ...
        case .month: return 1..<14                 // stable numbering, max = 13
        case .day: return 1..<31                   // Hebrew months max 30 days
        case .hour: return 0..<24
        case .minute: return 0..<60
        case .second: return 0..<60
        case .weekday: return 1..<8
        ...
        case .weekOfMonth:
            ...
            let upperBound = (30 + 6 + (7 - minimumDaysInFirstWeek)) / 7
            return lowerBound..<(upperBound + 1)
        case .weekOfYear: return 1..<56            // leap year has up to ~55 weeks
        case .yearForWeekOfYear: return 140742..<144684
        case .nanosecond: return 0..<1_000_000_000
        case .isLeapMonth: return 0..<1
        case .isRepeatedDay: return 0..<1
        case .dayOfYear: return 1..<386            // longest Hebrew year is 385 days
        case .calendar, .timeZone:
            return nil
        }
    }
    ...
    func range(of smaller: Calendar.Component, in larger: Calendar.Component, for date: Date) -> Range<Int>? {
        // Use the Gregorian-style algorithm-A: query ordinality at interval endpoints.
    ...
    func ordinality(of smaller: Calendar.Component, in larger: Calendar.Component, for date: Date) -> Int? {
        ...
        // For the common cases (day in year, day in month, month in year, nanosecond/second/minute/hour in day),
        // unpack the date once and compute directly.
        ...
        case (.day, .year):
            let preceding = Int(HebrewArithmetic.daysPrecedingCivilMonth(year: Int32(year), civilMonth: UInt8(civilMonth)))
            return preceding + day
        case (.day, .month):
            return day
        case (.month, .year):
            // Densify: in common years, civil-6 slot is empty, so month ordinality within year
            // is "number of real months from Tishrei through this one inclusive."
            if HebrewArithmetic.isLeapYear(Int32(year)) {
                return civilMonth
            } else {
                return civilMonth < 6 ? civilMonth : civilMonth - 1
            }
        case (.hour, .day):
            return hour + 1
        case (.minute, .hour):
    ...
    func dateInterval(of component: Calendar.Component, for date: Date) -> DateInterval? {
        ...
        switch component {
        case .era, .year:
            // Tishrei 1 of this year → Tishrei 1 of next year. Duration = year length
            // in local seconds (accounts for DST transitions within the year).
        ...
        case .weekOfYear, .weekOfMonth, .quarter, .yearForWeekOfYear,
             .isLeapMonth, .isRepeatedDay, .calendar, .timeZone:
            return nil
        }
    }
    ...
    func isDateInWeekend(_ date: Date) -> Bool {
        // Hebrew weekend: Saturday (Shabbat). Friday evening through Saturday
        // sunset in tradition; civil approximation is "weekday == 7 (Saturday)".
        // `dateComponents` returns weekday 1..7 (Sunday=1..Saturday=7).
        let comps = dateComponents([.weekday], from: date, in: self.timeZone)
        return comps.weekday == 7
    }
    ...
    func date(byAdding components: DateComponents, to date: Date, wrappingComponents: Bool) -> Date? {
        // Note: this implementation does a single pass across all fields, prioritizing
        // time → day-ish → month/year. For `wrappingComponents == true`, the intended
        // behavior is "stay within the containing unit"; our current implementation
        // approximates that via the same carryover path (adequate for perf benchmarks
        // and most common uses; full wrap semantics will be added later).

        var result = date

        // Time-of-day additions: simple TimeInterval arithmetic.
        if let ns = components.nanosecond, ns != 0 {
            result += Double(ns) / 1_000_000_000
        }
        if let s = components.second, s != 0 {
            result += Double(s)
        }
        if let m = components.minute, m != 0 {
            result += Double(m) * 60
        }
        if let h = components.hour, h != 0 {
            result += Double(h) * 3600
        }

        // Day-level additions collapse into a single day-count delta.
        var daysToAdd = 0
        ...
        if daysToAdd != 0 {
            // DST-aware fast path (O(1) regardless of N, no Hebrew arithmetic required).
            //
            // "Add N days" means "same local time-of-day, N civil days later". That's:
            //   1. Offset at current date: offset1
            //   2. Candidate UTC = current + N * 86400 seconds
            //   3. Offset at candidate: offset2 (might differ from offset1 if we crossed DST)
            //   4. Target = candidate - (offset2 - offset1)
            //
            // If offset1 == offset2 (no DST crossing), delta = 0 → pure + N*86400.
            // If we crossed DST (delta ≠ 0), we subtract the delta to land at the same
            // local clock time on the new civil day. Covers spring-forward (lose an hour,
            // so +24h in UTC lands at wrong local time; delta > 0 → subtract → correct UTC)
            // and fall-back (gain an hour; delta < 0 → subtract negative → add → correct UTC).
            //
            // This completely bypasses the Hebrew arithmetic + DateComponents round-trip
            // the previous implementation used. ~2 TimeZone dispatches per call, no heap
        ...
        // Year/month: unpack, adjust, repack. Uses stable (ICU) civil month numbering.
        ...
        if yearsToAdd != 0 || monthsToAdd != 0 {
            ...
            // Month addition: iterate one step at a time, respecting the skip at civil-6
            ...
            while remaining > 0 {
                ...
                if newCivilMonth > 13 {
                    ...
                    // Phantom Adar I slot in common years — skip straight to civil 7 (Adar).
                    newCivilMonth = 7
                }
                remaining -= 1
            }
            ...
            // After yearsToAdd, the target year may have a different leap status. If the
            // target month is 6 (Adar I) but the new year is common, demote to 7 (Adar).
    ...
    func dateComponents(_ components: Calendar.ComponentSet, from start: Date, to end: Date) -> DateComponents {
        // Minimal implementation: difference in seconds, populated across requested units.
        // This is good enough for arithmetic-scale uses but not a full Foundation-style
        // multi-unit difference (which requires recursively subtracting larger units first).
        // TODO: full multi-unit difference matching _CalendarGregorian.
        let tz = self.timeZone

        var result = DateComponents()

        let deltaSeconds = end.timeIntervalSince(start)
        let deltaDays = Int(deltaSeconds / 86400)

        if components.contains(.era) { result.era = 0 }
        if components.contains(.year) {
            let sc = dateComponents([.year], from: start, in: tz)
            let ec = dateComponents([.year], from: end, in: tz)
            result.year = (ec.year ?? 0) - (sc.year ?? 0)
        }
        if components.contains(.month) {
            // Approximate: total months between start and end. Rough pass using year+month.
            let sc = dateComponents([.year, .month], from: start, in: tz)
            let ec = dateComponents([.year, .month], from: end, in: tz)
            let sYear = Int32(sc.year ?? 0)
            let eYear = Int32(ec.year ?? 0)
            let yearsPart = Int(eYear - sYear) * 12
            result.month = yearsPart + (ec.month ?? 0) - (sc.month ?? 0)
        }
        if components.contains(.day) { result.day = deltaDays }
        if components.contains(.hour) { result.hour = Int(deltaSeconds / 3600) }
        if components.contains(.minute) { result.minute = Int(deltaSeconds / 60) }
        if components.contains(.second) { result.second = Int(deltaSeconds) }
        if components.contains(.nanosecond) {
            let frac = deltaSeconds - Double(Int(deltaSeconds))
            result.nanosecond = Int(frac * 1e9)
        }
        if components.contains(.weekday) { result.weekday = deltaDays % 7 }
        if components.contains(.weekdayOrdinal) { result.weekdayOrdinal = deltaDays / 7 }
        if components.contains(.weekOfYear) { result.weekOfYear = deltaDays / 7 }
        if components.contains(.weekOfMonth) { result.weekOfMonth = deltaDays / 7 }
