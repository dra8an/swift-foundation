...
internal final class _CalendarHebrew: _CalendarProtocol, @unchecked Sendable {
    ...
    func range(of smaller: Calendar.Component, in larger: Calendar.Component, for date: Date) -> Range<Int>? {
        ...
        // Use algorithm-A: query ordinality at interval endpoints.
    ...
    func date(byAdding components: DateComponents, to date: Date, wrappingComponents: Bool) -> Date? {
        ...
        // `.yearForWeekOfYear`: advance by N week-year lengths. ICU uses the number of
        // WEEKS in the source yearForWeekOfYear × 7 (50 or 51 weeks in common years,
        // 54-56 in leap years), NOT the calendar-year length. For Elul 20 5776 + 1
        // yearForWeekOfYear: +55 × 7 = +385 days = 2017-10-13 (ICU). For Adar 1 5785
        // + 1: +51 × 7 = +357 days = 2026-02-21 (ICU). Use numWeeksInYearForWeekOfYear().
        if let n = components.yearForWeekOfYear, n != 0 {
            let tz = self.timeZone
            // We use the date's yearForWeekOfYear (which accounts for year-boundary wraps),
